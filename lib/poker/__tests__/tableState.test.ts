...
describe("applyEvents", () => {
    ...
    for (let i = 0; i < hands.length - 1; i++) {
      ...
      for (const seat of current.seats) {
        const nextSeat = next.seats.find(
          (s) => s.playerName === seat.playerName,
        );
        if (!nextSeat) continue; // saiu da mesa — não tem "próxima mão" pra comparar
        expect(finalState.players.get(seat.playerName)?.stack).toBe(
          nextSeat.chips,
        );
      }
    }
  ...
  it("is deterministic for every prefix of the timeline (hand 1)", () => {
    ...
    for (let i = 0; i <= timeline.length; i++) {
      expect(applyEvents(hand, timeline.slice(0, i))).toEqual(
        applyEvents(hand, timeline.slice(0, i)),
      );
    }
  });
  ...
  it("never lets a streetBet go negative at any point in the replay (all hands)", () => {
    for (const hand of hands) {
      const timeline = buildTimeline(hand);
      for (let i = 0; i <= timeline.length; i++) {
        const state = applyEvents(hand, timeline.slice(0, i));
        for (const player of state.players.values()) {
          expect(player.streetBet).toBeGreaterThanOrEqual(0);
        }
      }
    }
  });
