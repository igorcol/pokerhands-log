...
function groupAmbientBySection(
  events: AmbientEvent[],
): Map<AmbientEvent["section"], AmbientEvent[]> {
  ...
  for (const street of STREET_ORDER) {
    ...
    if (street !== "preflop") {
      events.push({
        kind: "street",
        street,
        cards: newCardsForStreet(hand.board, street),
      });
    }
    ...
    for (const uncalled of hand.uncalledBets.filter(
      (u) => u.street === street,
    )) {
      events.push({
        kind: "uncalled-return",
        player: uncalled.player,
