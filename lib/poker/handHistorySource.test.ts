...
import { handHistoryDir, listAccounts, readAccountHands } from './handHistorySource'
...
describeReal('handHistorySource (pasta real)', () => {
  ...
  it('serves the second read from cache (same object identity)', () => {
    const [account] = listAccounts()
    expect(readAccountHands(account).hands[0]).toBe(readAccountHands(account).hands[0])
  })
})
