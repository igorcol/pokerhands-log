...
function deriveOutcome(hand: Hand, hero: string): HandOutcome {
  const folded = hand.actions.find((action) => action.player === hero && action.type === 'fold')
  if (folded) return { kind: 'folded', street: folded.street }

  const showedDown = hand.reveals.some((r) => r.player === hero && r.source === 'showdown')
  if (!showedDown) return { kind: 'won-without-showdown', street: finalStreet(hand.board) }

  if (!hand.winners.some((w) => w.player === hero)) return { kind: 'showdown-lost' }
  return hand.winners.length > 1 ? { kind: 'showdown-split' } : { kind: 'showdown-won' }
}
...
export function deriveHandResult(hand: Hand): HandResult | null {
  ...
  if (!seat) {
    throw new Error(`Hero ${hero} was dealt cards but has no seat in hand ${hand.id}`)
  }

  const position = derivePositions(hand).get(hero)
  if (!position) {
    throw new Error(`Hero ${hero} has no derivable position in hand ${hand.id}`)
  }
