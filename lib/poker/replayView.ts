...
export function seatLayout(hand: Hand): SeatSlot[] {
  ...
  const heroSeat = hero ? hand.seats.find((s) => s.playerName === hero) : undefined
...
export function activePlayer(timeline: ReplayEvent[], frame: number): string | null {
...
function phaseOfEachEvent(timeline: ReplayEvent[]): ReplayPhase[] {
  ...
  return timeline.map((event) => {
    ...
    if (event.kind === 'reveal' || event.kind === 'collect') current = 'showdown'
    return current
  })
}
...
export function streetSegments(timeline: ReplayEvent[]): StreetSegment[] {
  ...
  phaseOfEachEvent(timeline).forEach((phase, index) => {
    ...
    segments.push({ phase, startFrame: index, endFrame: index + 1, eventCount: 1 })
  })
